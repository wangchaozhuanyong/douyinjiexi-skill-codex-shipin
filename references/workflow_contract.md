...
13.4. If HyperFrames PNG sequence is used, run `scripts/write_hyperframes_render_profile.py --project <project>` before export and write `hyperframes_render_profile.json`; the profile must use `render_mode=png_sequence`, `worker_count=1`, `max_worker_count=1`, and `protocol_timeout_ms >= 900000`.

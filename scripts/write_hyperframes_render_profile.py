...
def selected_template_profile(project: Path) -> dict[str, Any]:
    selection = load_json(project / "internal" / "fixed_template_selection.json")
    scene_motion = selection.get("scene_motion_templates")
    if not isinstance(scene_motion, dict):
        return {}
    profile = scene_motion.get("hyperframes_render_profile")

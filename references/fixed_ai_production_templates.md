...
本文件把可固定的高成本设计决策收敛成四类模板库：背景板、转场与音效、前景组件、男声混音。它们的目标不是让所有视频长得一样，而是减少每次临场重新设计造成的 token 浪费、质量波动和风格冲突。
...
- 封面成图：`references/fixed_ai_cover_template_rotation.json`
- 背景成图：`assets/ai_background_templates_fixed/`

## 固定与随机的边界

- 封面成图固定为真实图片资产，按尺寸池顺序轮换。

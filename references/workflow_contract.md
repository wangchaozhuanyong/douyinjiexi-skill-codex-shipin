...
12.1. AI knowledge videos must select the fixed safe cover template before HyperFrames render when the cover is intended to be the first frame. `internal/first_frame_cover.png`, `internal/cover.png`, `internal/publish_cover_text.txt`, and `internal/publish_cover_report.json` must exist before final timeline rendering; the composition must place `first_frame_cover.png` as scene 0 / the first visual frame. If a one-off custom cover is approved instead, it must meet the same first-frame and text-compliance rules.
...
20.1.1. The user has granted standing authorization for routine Qingdou/Douyin publish-chain browser actions, including ordinary slider or image security verification when the active environment permits agent handling. If Qingdou shows a normal slider/image check, attempt to complete it and continue the exact title/caption/topics check; do not require the user to handle it manually by default. Stop only for SMS verification, phone-only login recovery, real-name/account-owner checks, verification that clearly must be done by the user, or any active browser/tool safety rule that requires fresh action-time confirmation. Never bypass verification, guess codes, store credentials, cookies, or verification data, and never mark Qingdou as passed without visible passed evidence.
...
25. Provider Usage Audit -> `provider_usage_audit.json`
26. Qingdou Keyword Check -> `qingdou_keyword_check.json`
27. Publish Contract -> `publish_contract.json`
28. Pre-Publish Gate -> `scripts/pre_publish_gate.py` sets `publish_contract.gate.status`
29. Promote Final -> `final/final.mp4` only if the publish contract gate passed

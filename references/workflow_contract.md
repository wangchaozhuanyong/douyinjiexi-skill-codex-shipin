...
0.3.1.2. `audio_music_decision` is required before storyboard, TTS, HyperFrames, BGM selection, or audio mix. It must record `music_policy` (`required_bgm`, `library_music_bgm`, `voice_only_clean`, or `no_bgm`), `bgm_source_priority`, `voice_policy`, `voice_priority`, `sfx_required=false` by default, `mix_note`, `generated_bgm_allowed=false`, `generated_background_audio_allowed=false`, and `reference_bgm_policy`. Scheme 1 and Scheme 7 default to `required_bgm` only when they are music-led/no-voice; Scheme 2, Scheme 3, Scheme 4, Scheme 5 narrated mode, and Scheme 6 default to `voice_only_clean`. Reference-led audio may override the default only when documented in reference analysis. If a user-provided Douyin reference has BGM and the final is for Douyin, use the same reference music or same Douyin music-page track when technically possible; if unavailable, stop for the user's music file, platform same-music selection, or explicit no-BGM approval. Explicit no-music user requests set `music_policy=voice_only_clean` for narrated videos. Do not generate, synthesize, self-create, or silently substitute BGM, SFX beds, noise beds, ambience, electric buzz, or texture audio.
...
7. `reference_analysis.json` is required when the user provides a reference video/link/share text. For video references, this report must prove the playable video body was obtained and inspected, with local video path or blocker, duration, first 3-5 second evidence, contact sheet/detail frames when technically possible, and music/voice relationship. A title, thumbnail, share caption, URL metadata, music page, or author page alone does not satisfy this gate.
...
13.2. Background audio for this user's AI/Douyin knowledge videos must be one of two modes before final QA: `voice_only_clean` with no background sound, or `library_music_bgm` using approved music-library/local/reference music evidence. Generated/self-created music, continuous SFX beds, noise beds, ambience, electric buzz, whoosh beds, texture beds, or `sfx-bed.wav`-style background audio are blocking. If metadata, storyboard, fixed template selection, or mix reports mention generated/self-created background audio, do not run final QA, do not promote, and do not publish.
...
20.3.3. Before building or accepting `publish_contract.json`, run `scripts/ai_video_workflow_guard.py --project <project> --phase publish` and require `internal/workflow_guard.json.status="passed"`. `scripts/produce_ai_video.py` must run it before `build_publish_contract.py`; `scripts/pre_publish_gate.py` must refresh it again before evaluating the contract. The guard blocks skipped workflow artifacts, missing manual frame-review/contact-sheet evidence, weak visual-review scores, missing dynamic background/template inheritance, missing center-crop cover proof, missing Qingdou/text evidence, and `scheme_7_ai_hot_rank_top5` projects without exactly five scored source-backed rank items. Do not create this report by hand after the fact; it must be generated from the current project artifacts.
...
30. Workflow Guard -> `workflow_guard.json`
31. Publish Contract -> `publish_contract.json`
32. Pre-Publish Gate -> `scripts/pre_publish_gate.py` refreshes `workflow_guard.json` and sets `publish_contract.gate.status`
33. Promote Final -> `final/final.mp4` only if the publish contract gate passed
34. Optional SAU Douyin Upload -> `scripts/douyin_sau_publish.py` writes `douyin_sau_upload_report.json`; dry-run first, then `--execute` only when the user-authorized publish route and schedule/immediate policy are explicit

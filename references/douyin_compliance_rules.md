...
The user has granted standing authorization for routine Qingdou/Douyin publish-chain browser actions, including text entry, paste/replace, clicking the check button, reading the visible result, and ordinary slider or image security verification when the current environment permits agent handling. Execute these routine steps directly; do not hand the copy back to the user for manual paste or manual checking. Reuse the user's current logged-in Chrome tab/session, and close any extra tab/window opened for the task after completion. Current-environment browser automation is allowed for page input and visible-result reading, but never extract, store, or reuse credentials, cookies, SMS codes, verification codes, or verification data. The default Qingdou route is `scripts/qingdou_browser_check.py --project <project> --mode prepare --set-clipboard`, then running the prepared bookmarklet in the current logged-in Qingdou tab so the page clears the rich-text editor, inserts the exact publish text, verifies text equality, and clicks the check button. Chrome may strip a pasted `javascript:` prefix; type that ASCII prefix manually or paste `qingdou_check_text.txt` into the empty Qingdou input and verify the visible character count before clicking the page button. Only after reading a visible `未检查到敏感词` result may `record-passed` write `qingdou_keyword_check.json`; if Qingdou only flags `抖音` inside the fixed `#我在抖音聊科技` hashtag, use `record-user-approved-topic --term 抖音`; otherwise write `record-blocked` with the exact blocker. Still stop for SMS verification, phone-only login recovery, real-name/account-owner checks, verification that clearly must be completed by the user, or any active browser/tool safety rule requiring fresh action-time confirmation. Never bypass verification, guess codes, or mark Qingdou as passed without visible passed evidence.
...
Standing topic override: the user has explicitly confirmed on 2026-06-21 that `#我在抖音聊科技` must always be kept and must not be asked about again. If Qingdou only flags `抖音` inside this exact hashtag, record `status: "user_override_accepted"`, the fixed hashtag, and this standing approval, then continue. This is not a Qingdou pass and must not be applied to any other topic, title, caption body, subtitle, cover, on-screen text, contact info, guarantee, station-out diversion, or additional Qingdou hit.

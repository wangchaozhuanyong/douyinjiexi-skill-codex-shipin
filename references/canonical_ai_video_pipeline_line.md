...
封面文字也在这一步定稿。封面不是发布标题全文，必须写成适合抖音封面预览的短主题：

- 主标题建议 8 到 14 个中文字符，最多 18 个。
- 副标题建议 6 到 12 个字符。
- 16:9 视频的封面文字必须放在中间 9:16 裁切也能完整看到的位置。
- 如果完整标题太长，封面文字要改写成短句，不能缩小字号硬塞。
...
- 每个镜头至少输出 1 张代表样帧。
- 封面输出横版完整图、竖版兼容图、抖音中间裁切预览图。
- 检查文字是否溢出、模块是否规整、前景设计是否像本条内容，不像通用模板。
- 样帧通过后，才允许导出完整图片序列和 FFmpeg 合成。
...
- 固定背景层、低频光效层、半静态装饰层要缓存。
- 每帧只重画当前镜头真正变化的前景模块、字幕和状态节点。
- 记录 `frame_sequence_export_report.json.elapsed_sec`。
- 如果 60 到 75 秒视频超过 10 分钟，写入 `production_bottleneck_log.json`，后续优先优化缓存层。
...
16:9 AI 视频封面要同时满足三份输出：

- `cover.png`：完整 16:9 封面。
- `cover_publish_vertical.png`：竖版兼容封面。
- `cover_publish_douyin_center_crop.png`：抖音中间裁切预览。

公开封面文字必须在 `cover_publish_douyin_center_crop.png` 里完整可见。这个预览不过，不能进入发布前检查。
...
- 公开内容边界：Qingdou/青豆、QA、合规检查、发布检测、上传前通过、敏感词检测等只属于后台检查和内部报告；不得写进公开视频的标题、开头钩子、脚本、字幕、封面字、画面文字、发布文案或话题，除非当天最终选题本身就是内容合规工具。每日 AI 资讯/技巧视频必须先围绕最新市场资讯和 AI/Codex/ChatGPT/Gemini 高级实用技巧展开；一旦公开视频案出现内部检查流程词，`copy_and_compliance_gate` 必须判定需要改写，不能进入 TTS、渲染或发布。
- 本地合规先过。

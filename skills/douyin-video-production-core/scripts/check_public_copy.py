...
DEFAULT_TERM_BANK = Path(__file__).resolve().parents[1] / "references" / "forbidden_terms_learning_bank.jsonl"
...
RISK_PATTERNS: list[dict[str, str]] = [
    ...
    {
        "level": "error",
        "category": "external_diversion",
        "pattern": r"微信|VX|vx|QQ|网址|链接|URL|https?://|复制.{0,6}(链接|网址)|打开.{0,8}(网站|网页|页面|官网|某站|链接|网址)|访问.{0,8}(网站|网页|官网|链接|网址)|进入.{0,8}(官网|网站|网页)|加群|扫码|私信|私信领取|联系方式|领取|下载|电话[:：]?\\d",
    },
...
SUGGESTIONS = {
    ...
    "external_diversion": "删除站外引流、链接/网址、联系方式、领取下载和私域导流表达；Skill 介绍改写成输入内容、解决目的、输出结果。",
...
def main() -> int:
    ...
    fingerprint_inputs = [Path(path) for path in checked_files if path != "stdin"]
    if fingerprint_inputs:
        write_report_with_fingerprints(report, fingerprint_inputs)
